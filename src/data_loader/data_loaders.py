...
import torch
import numpy as np
from data_loader.dataset.pas_dataset import PASDataset, PASDatasetForEncDec
...
class PASDatasetForEncDecLoader(DataLoader):
    def __init__(self,
        ...
        init_kwargs = {
            ...
            'batch_size': batch_size,
            'shuffle': shuffle,
            'collate_fn': broadcast_collate_fn_for_enc_dec,
            'num_workers': num_workers,
            'pin_memory': pin_memory,
        }
        super().__init__(**init_kwargs)
...
def broadcast_collate_fn_for_enc_dec(batch: List[Tuple[np.ndarray, ...]]) -> Dict[str, torch.Tensor]:
    input_ids, attention_mask, decoder_input_ids, decoder_attention_mask, ng_token_mask, target, deps, task, overt_mask = zip(*batch)  # Tuple[list]
    ng_token_mask = np.broadcast_arrays(*ng_token_mask)
    target = np.broadcast_arrays(*target)
    deps = np.broadcast_arrays(*deps)
    inputs = (input_ids, attention_mask, decoder_input_ids, decoder_attention_mask, ng_token_mask, target, deps, task, overt_mask)
    labels = ('input_ids', 'attention_mask', 'decoder_input_ids', 'decoder_attention_mask', 'ng_token_mask', 'target', 'deps', 'task', 'overt_mask')
