...
# copied from transformers 2.10.0
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, **_):
...
class ConditionalBertEmbeddings(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    ...
    def __init__(self, config, **kwargs):
        ...
        self.l_arg = nn.Linear(config.hidden_size, config.hidden_size * self.num_case)
    ...
    def forward(self,
        ...
        if inputs_embeds is None:
            inputs_embeds = self.word_embeddings(input_ids)  # (b, seq, hid)
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        embeddings = inputs_embeds + position_embeddings + token_type_embeddings  # (b, seq, hid)
...
class ConditionalBertSelfAttention(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(self,
        ...
        query_layer = self.transpose_for_scores(mixed_query_layer)  # (b, heads, seq, hsize)
        ...
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)  # (b, seq, hid)

        outputs = (context_layer, attention_probs) if self.output_attentions else (context_layer,)
        return outputs


class CaseAwareBertSelfAttention(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(config, "embedding_size"):
            raise ValueError(
    ...
    def forward(self,
        ...
        mixed_case_value_layer = self.case_value(hidden_states)  # (b, seq, case*hid)
        # (b, seq, case, hid)
        case_value_layer = mixed_case_value_layer.view(batch_size, seq_len, self.num_case, self.all_head_size)
        # (b, seq, case, hid), (b, seq, case, seq) -> (b, seq, hid)
        case_context_layer = torch.einsum('bjcs,bicj->bis', case_value_layer, rel_weights)
        context_layer += case_context_layer
    ...
    def forward(
        ...
        **kwargs
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, **kwargs
        )
...
class BertLayer(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.attention = BertAttention(config, **kwargs)
    ...
    def forward(
        ...
        **kwargs
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, **kwargs)
...
class BertEncoder(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.layer = nn.ModuleList([BertLayer(config, **kwargs) for _ in range(config.num_hidden_layers)])
        ...
        **kwargs
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, **kwargs
            )
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, **kwargs):
        ...
        if kwargs.get('conditional_bert_embeddings', None) is True:
            self.embeddings = ConditionalBertEmbeddings(config, **kwargs)
        else:
            self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config, **kwargs)
