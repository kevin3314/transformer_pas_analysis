...
class CoreferenceAwareModel2(BaseModel):
    """
    出力層で共参照の結果を与える
    hard な重みで、それぞれの格の重みで変換
    """
    ...
    def forward(self,
        ...
        eye = torch.eye(seq_len, dtype=torch.bool, device=input_ids.device)  # (seq)
        hard_annealed_out_coref = eye[annealed_out_coref.argmax(dim=2)]

        context = torch.einsum('bjch,bij->bich', h_a, hard_annealed_out_coref.float())  # (b, seq, case, hid)
        h_a += context
...
class CoreferenceAwareModel4(BaseModel):
    """
    出力層で共参照の結果を与える
    soft な重みで、別に用意した case' の重みで変換
    """
    ...
    def forward(self,
        ...
        context = torch.einsum('bjch,bij->bich', h_a2, annealed_out_coref.softmax(dim=2))  # (b, seq, case, hid)
        h_a += context
...
class CoreferenceAwareModel5(BaseModel):
    """
    CAModel4 に加え，初段の coref も loss を計算する
    """
...
class CoreferenceAwareModel6(BaseModel):
    """
    CAModel5 に加え，self.outに別の重みを使用する
    """
    ...
    def __init__(self,
        ...
        self.l_arg2 = nn.Linear(bert_hidden_size, bert_hidden_size * self.num_case)
        self.out = nn.Linear(bert_hidden_size, 1, bias=False)
        self.out2 = nn.Linear(bert_hidden_size, 1, bias=False)
...
class CoreferenceAwareModel7(BaseModel):
    """
    CAModel4 の annealing しないバージョン
    """
    ...
    def forward(self,
        ...
        # -> (b, seq, case*hid) -> (b, seq, case, hid)
        h_a2 = self.l_arg2(self.dropout(sequence_output)).view(batch_size, seq_len, self.num_case, -1)
        context = torch.einsum('bjch,bij->bich', h_a2, out_coref.detach().softmax(dim=2))  # (b, seq, case, hid)
        h_a += context
...
CoreferenceSeparatedModel = CoreferenceAwareModel3


class CoreferenceSeparatedModel2(BaseModel):
    """
    BaseModel (corefonly) の結果を読む
    Coreference-aware BaseModel のほうでも coref を解いていた．ダメ
    """
...
class CoreferenceSeparatedModel3(BaseModel):
    """
