...
def test_ne(fixture_kwdlc_readers: List[KWDLCReader]):
    kwdlc_reader = fixture_kwdlc_readers[2]
    nes = kwdlc_reader.named_entities
    assert len(nes) == 2
    ne = nes[0]
    ...
    kwdlc_reader = fixture_kwdlc_readers[3]
    nes = kwdlc_reader.named_entities
    assert len(nes) == 3
    ne = nes[0]
